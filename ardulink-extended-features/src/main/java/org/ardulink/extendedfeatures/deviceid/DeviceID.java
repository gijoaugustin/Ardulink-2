 ...
 * This XFeature is able to retrieve the Unique ID from an Arduino based board. If arduino doesn't
 * have an unique id this class suggests an id. Then in the reply message between parameters
...
public class DeviceID implements XFeature, RplyListener {
	...
	public DeviceID(AbstractListenerLink link, long timeout, TimeUnit timeUnit) {
		...
		checkState(timeout > 0, "Timeout has to be greater than 0. It is: %s", timeout);
	...
	private void initUniqueID() throws IOException {
		if(messageReplyID == null) {
			...
			checkState(event.hasParameters(), "Returned message hasn't parameters");
			
			uniqueID = (String)event.getParameters().get("UniqueID");
			checkNotNull(uniqueID, "Returned message hasn't an UniqueID parameter");
		}
	}
		...
		try {
			...
			try {
				...
				checkState(messageReplyID == event.getId(),
						"Waited for %s but got %s", messageReplyID, event.getId());
